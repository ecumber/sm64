...
void bhv_goomba_update(void) {
    ...
    if (obj_update_standard_actions(o->oGoombaScale)) {
        ...
        // FIXED ! @bug Weak attacks on huge goombas in a triplet mark them as dead even if they're not.
        ...
        // obj_handle_attacks returns the index of the attack handler, not the actual attack type, which is why comparing against just 'attack' doesn't work.
        if ( (attack != 0) && (sGoombaAttackHandlers[o->oGoombaSize & 1][attack - 1] != ATTACK_HANDLER_SPECIAL_HUGE_GOOMBA_WEAKLY_ATTACKED)) {
            mark_goomba_as_dead();
        }
